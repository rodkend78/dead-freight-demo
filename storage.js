...
(function (root) {
  'use strict';
  const limits = {salvage:100000000,extractions:100000000,manifests:100000000,medicalBag:1,ammoRig:1,contractsCompleted:100000000,ironworksClears:100000000,prototypeCores:100000000,stabilizer:1,overdrive:1,equippedMod:2,guideMode:2,utilityChoice:2,smokeUnlocked:1,utilityRig:1,depotBlueprints:100000000,campaignStage:3,quietKit:1,loaderKit:1,sorterKit:1,equippedKit:3};
  function inspectBackup(text) {
    ...
    if (!data || Array.isArray(data) || data.version!==1) throw Error('This game can restore stash version 1 only.');
    if (!Array.isArray(data.weapons) || data.weapons.length!==5 || data.weapons.some(n=>!Number.isInteger(n)||n<0||n>999)) throw Error('The backup must contain five valid weapon counts.');
    ...
    return {json:text,version:1,salvage:n('salvage'),extractions:n('extractions'),campaignStage:n('campaignStage'),weapons:data.weapons.slice()};
  }
